...
class SingleResponsibility {
    ...
    class SessionHandler {
        fun startSession(user: User) {
            println("Starting ${user.name} session with ${user.id} id.")
        }

        fun activateUserAccount(user: User) {
            println("Activating ${user.name}.")
        }

        fun checkAccessRights(user: User) {
            println("Checking ${user.name} access.")
        }
    }
